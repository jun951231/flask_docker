...
class Population(object):
    ...
    def read_data(self):
        data = csv.reader(open('../population/data/202106_202106_연령별인구현황_월간.csv', 'rt', encoding='UTF-8'))
        next(data)
...
class PopulationWithPandas(object):
    ...
    def read_data(self):
        df = pd.read_csv('../population/data/202106_202106_연령별인구현황_월간.csv', encoding='UTF-8', thousands =',', index_col = 0)
        df.to_csv('./data/202106_202106_연령별인구현황_월간_without_comma.csv', sep=',', na_rep='NaN')
        data = csv.reader(open('../population/data/202106_202106_연령별인구현황_월간_without_comma.csv', 'rt', encoding='UTF-8'))
        next(data)
        self.data = list(data)
